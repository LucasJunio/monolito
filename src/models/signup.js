                        ...
                        bcrypt.hash(payload.senha, salt, function (err, hash) {
                            ...
                            request.query(querysql, async function (err, recordset) {   
                                ...
                                sendEmail(payload.usuario, (err, result) => {
                                    if (!result.success) {
                                        callback({ message: 'usuário cadastrado, mas email não enviado.', err }, false)
                                        return;
                                    } else {
                                        sendSms(payload.usuario, (err, result) => {
                                            if (!result.success) {
                                                callback({ message: 'usuário cadastrado, email enviado, mas sms não enviado.', err }, false)
                                                return;
                                            } else {
                                                return callback(null, { token: result.token, success: true })                                                
                                            }
                                        })
                                    }
                                })
