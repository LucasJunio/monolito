...
const validationMiddleware = async (object, callback) => {
  ...
  if(account.error) error.push(account.error.details.message)
  const address = await addressSchema.validate(object.endereco_cnpj)
  if(address.error) error.push(address.error.details.message)
  const addressCpf = await addressSchema.validate(object.endereco_cpf)
  if(addressCpf.error) error.push(addressCpf.error.details.message)
  const enterprise = await enterpriseSchema.validate(object.empresa)
  if(enterprise.error) error.push(enterprise.error.details.message)
